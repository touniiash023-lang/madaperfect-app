...
/**
 * App.js - Version nettoyée et complète (Option A1)
 *
 * - Recherche produit / client / facture (barres de recherche)
 * - Upload images -> Storage -> sauvegarde d'URL dans Firestore
 * - Modal preview d'image (lightbox)
 * - Gestion rôle simple via token claims (onAuthStateChanged -> getIdTokenResult)
 * - PDF (jsPDF) génération
 *
...
export default function App() {
  ...
  const [role, setRole] = useState(null); // 'superadmin'|'admin'|'commercial'|null
  ...
  // UI states
  const [view, setView] = useState("dashboard"); // dashboard|articles|invoices|clients|settings
  ...
  const [invoiceDraft, setInvoiceDraft] = useState({
    id: null, clientId: null, items: [], type: 'commercial', date: dayjs().format('YYYY-MM-DD'), deliveryDate: '', deliveryAddress: '', paid: 0, number: ''
  });
  const [editingInvoice, setEditingInvoice] = useState(false);

  // file upload buffer (FileList or array)
  ...
  // dashboard derived
  const calcTotal = inv => (inv.items || []).reduce((s,it)=> s + cPrice(it.price) * Number(it.qty||0), 0);
  ...
  const ticketMoyen = invoices.length ? Math.round(invoices.reduce((s,inv)=> s + calcTotal(inv), 0) / invoices.length) : 0;
  ...
  invoices.forEach(inv => { totalSum += calcTotal(inv); paidSum += Number(inv.paid || 0); });
  const pieData = [{ name: "Payé", value: paidSum }, { name: "Restant", value: Math.max(0, totalSum-paidSum) }];
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        console.warn("Error getting token claims:", err);
        setRole(null);
      }
      // load data after auth (so Firestore rules that depend on auth may pass)
      await loadAll();
    });
    return () => unsub();
  ...
  async function loadAll() {
    await Promise.allSettled([loadProducts(), loadClients(), loadInvoices()]);
  }
  ...
  function handleProductImages(e) {
    ...
    if (!files) return;
    setProductFiles(Array.from(files));
    // show preview thumbnails in form from files? we push only urls after upload; keep files in state
  }
  ...
  async function uploadFilesAndGetUrls(files) {
    ...
    const urls = [];
    for (let f of files) {
      const name = `products/${Date.now()}-${f.name}`;
  ...
  async function saveProductFirestore() {
    ...
    try {
      // upload files first
      const uploaded = await uploadFilesAndGetUrls(productFiles);
      const images = [...(productForm.images || []), ...uploaded];
      ...
      // reset form
      setProductForm({ id: null, name: "", price: "", description: "", link: "", images: [] });
      setProductFiles([]);
      await loadProducts();
  ...
  async function saveClientFirestore() {
    ...
    if (!isAdmin()) return alert("Action non autorisée");
    const payload = { name: clientForm.name, email: clientForm.email || "", phone: clientForm.phone || "", address: clientForm.address || "" };
    if (clientForm.id) {
      await updateDoc(doc(db, "clients", clientForm.id), payload);
    } else {
  ...
  function resetInvoiceDraft() {
    setInvoiceDraft({ id: null, clientId: null, items: [], type: 'commercial', date: dayjs().format('YYYY-MM-DD'), deliveryDate: '', deliveryAddress: '', paid: 0, number: '' });
  }
  ...
  function generateInvoicePDF(inv) {
    ...
    const fPrice = v => formatMG(Number(v||0));
    ...
    doc.text("NIF 50T3775350    STAT 461011120240076695", margin, y); y += 14;
    doc.text("Adresse: Lot IAH 50 B Itaosy", margin, y); y += 14;
    ...
    (inv.items || []).forEach(it => {
      ...
      const text = doc.splitTextToSize(it.name || "", 170);
      const lineHeight = 14;
      const textHeight = text.length * lineHeight;
      const rowHeight = Math.max(22, textHeight + 10);
      if (y + rowHeight > 820) { doc.addPage(); y = margin; }
    ...
    // totals box
    y += 12;
    const boxX = 350, boxW = 210, split = boxX + 70;
    ...
    doc.setFont("helvetica","normal");
    doc.text(formatMG(total), split + 10, y + 18);
    doc.text(formatMG(paid), split + 10, y + 38);
    doc.text(formatMG(rest), split + 10, y + 58);
  ...
  useEffect(() => {
    // if user is already authenticated, data is loaded in the onAuthStateChanged handler
    // but allow manual load when dev (no auth)
    // loadAll(); // don't auto-load if security rules require auth
  }, []);

  /* ---------- RENDU ---------- */
  if (!user) return <Login onLogin={() => {/* handled by firebase auth in Login component */}} />;
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <div className="flex">
        <aside className="w-72 bg-indigo-900 text-white min-h-screen p-6">
          <h1 className="text-2xl font-bold mb-6">Mada Perfect Import</h1>
          ...
          {view === 'dashboard' && (
            <div className="space-y-6">
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  <div className="bg-white p-4 rounded shadow">
                    ...
                    <div className="text-xs text-gray-600">{formatMG(paidSum)} payé • {formatMG(totalSum - paidSum)} restant</div>
                  </div>
              ...
              <div className="bg-white p-4 rounded shadow">
                ...
                {todayInvoices.length === 0 ? <div className="text-gray-500">Aucune facture aujourd'hui.</div> : (
                  <div className="space-y-2">
                      ...
                      return (
                        <div key={inv.id} className="flex items-center justify-between border rounded p-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button className="px-2 py-1 bg-yellow-600 text-white rounded" onClick={() => generateInvoicePDF(inv)}>PDF</button>
                            <button className="px-2 py-1 border rounded" onClick={() => startEditInvoice(inv)}>Voir</button>
                          </div>
                        </div>
                      );
                    })}
          ...
          {view === 'articles' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
                ...
                <input type="text" placeholder="🔍 Rechercher un produit..." value={searchProduct} onChange={(e) => setSearchProduct(e.target.value)} className="w-full px-4 py-2 border rounded-lg mb-6 shadow-sm" />
                <div className="space-y-6">
                  {filteredProducts.length === 0 ? <div className="text-center py-6 text-gray-500 italic">Aucun produit trouvé.</div> : filteredProducts.map(p => {
                      ...
                      <div key={p.id} className="flex items-start justify-between p-4 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition border border-gray-200">
                        <div className="flex gap-4">
                          <div className="flex flex-col gap-2">
                            {imagesArray.length > 0 ? imagesArray.slice(0,3).map((img,i) => (
                              <img key={i} src={img} onError={(e)=> e.target.src="https://via.placeholder.com/80"} className="w-24 h-24 object-cover rounded-lg shadow cursor-pointer hover:scale-105 transition" alt="product" onClick={() => setPreviewImage(img)} />
                            )) : <img src="https://via.placeholder.com/80" className="w-24 h-24 object-cover rounded-lg opacity-60" alt="placeholder" />}
                          </div>
                          <div>
                            <h4 className="text-xl font-bold text-gray-800">{p.name}</h4>
                            <p className="text-gray-600 max-w-md mt-1">{p.description || "Aucune description fournie"}</p>
                            {p.link && <a href={p.link} target="_blank" rel="noreferrer" className="text-indigo-600 text-sm underline mt-2 inline-block">Voir lien fournisseur</a>}
                            {imagesArray.length > 0 && <button onClick={() => setPreviewImage(imagesArray[0])} className="text-blue-600 underline ml-4">Voir image</button>}
                          </div>
                        </div>
                        <div className="text-right min-w-[140px]">
                          <div className="text-xl font-bold text-indigo-700 mb-3">{formatMG(p.price)}</div>
                          <div className="flex flex-col gap-2">
                            ...
                            })} className="px-3 py-1 bg-yellow-400 rounded" disabled={!isAdmin()}>Modifier</button>
                            <button onClick={() => deleteProductFirestore(p.id)} className="px-3 py-1 bg-red-500 text-white rounded" disabled={!isAdmin()}>Supprimer</button>
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
                <h3 className="text-xl font-bold mb-5">{productForm.id ? "Modifier le produit" : "Ajouter un produit"}</h3>
                <input value={productForm.name} onChange={(e)=> setProductForm(f=>({...f, name: e.target.value}))} placeholder="Nom du produit" className="w-full px-3 py-2 border rounded-lg mb-3" />
                <input value={productForm.price} onChange={(e)=> setProductForm(f=>({...f, price: e.target.value}))} placeholder="Prix" className="w-full px-3 py-2 border rounded-lg mb-3" />
                <textarea value={productForm.description} onChange={(e)=> setProductForm(f=>({...f, description: e.target.value}))} placeholder="Description" className="w-full px-3 py-2 border rounded-lg mb-3 h-28" />
                <input value={productForm.link} onChange={(e)=> setProductForm(f=>({...f, link: e.target.value}))} placeholder="Lien fournisseur" className="w-full px-3 py-2 border rounded-lg mb-3" />
                <label className="font-semibold text-gray-700">Images :</label>
                <input type="file" multiple onChange={handleProductImages} className="w-full px-3 py-2 border rounded-lg mb-3 bg-white" />
                <div className="flex flex-wrap gap-3 mb-4">
                  {(productForm.images || []).map((img, i) => (
                    <div key={i} className="relative">
                      <img src={img} className="w-20 h-20 object-cover rounded-lg border shadow" alt="preview" />
                      <button onClick={() => setProductForm(f => ({ ...f, images: f.images.filter((_, idx) => idx !== i) }))} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-6 h-6 text-xs">×</button>
                    </div>
                  ))}
                </div>
                <div className="flex gap-2">
                  <button onClick={saveProductFirestore} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg">Enregistrer</button>
                  <button onClick={()=> setProductForm({ id:null, name:'', price:'', description:'', link:'', images:[] })} className="px-4 py-2 border rounded-lg">Annuler</button>
                </div>
              </div>
            </div>
          )}
          ...
          {view === 'clients' && (
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-2 bg-white p-4 rounded shadow">
                ...
                <input type="text" placeholder="🔍 Rechercher un client..." value={searchClient} onChange={(e)=> setSearchClient(e.target.value)} className="w-full px-3 py-2 border rounded mb-4 shadow-sm" />
          ...
          {view === 'invoices' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
                ...
                {/* Client search + pick */}
                <label>Client</label>
                <input type="text" placeholder="Rechercher un client..." value={searchClient} onChange={(e)=> setSearchClient(e.target.value)} className="w-full px-3 py-2 border rounded mb-2" />
                <div className="bg-white border rounded shadow max-h-40 overflow-y-auto mb-4">
                  {clients.filter(c => (c.name || "").toLowerCase().includes((searchClient || "").toLowerCase())).map(c => (
                    <div key={c.id} onClick={() => { setInvoiceDraft(d=>({...d, clientId: c.id})); setSearchClient(c.name); }} className="px-3 py-2 hover:bg-indigo-100 cursor-pointer">{c.name}</div>
                  ))}
                </div>

                {/* Product search + add */}
                <label>Ajouter produit</label>
                <input type="text" placeholder="Rechercher un produit..." value={searchProduct} onChange={(e)=> setSearchProduct(e.target.value)} className="w-full px-3 py-2 border rounded mb-2" />
                <div className="bg-white border rounded shadow max-h-40 overflow-y-auto mb-4">
                  {products.filter(p => (p.name||"").toLowerCase().includes((searchProduct||"").toLowerCase())).map(prod => (
                    <div key={prod.id} onClick={() => { setInvoiceDraft(d=>({...d, items: [...d.items, { id: Date.now(), productId: prod.id, name: prod.name, qty:1, price: prod.price }]})); setSearchProduct(""); }} className="px-3 py-2 hover:bg-indigo-100 cursor-pointer">{prod.name}</div>
                  ))}
                </div>
              ...
              <div className="bg-white p-4 rounded shadow">
                ...
                <input type="text" placeholder="🔍 Rechercher une facture..." value={searchInvoice} onChange={(e)=> setSearchInvoice(e.target.value)} className="w-full px-3 py-2 border rounded mb-3 shadow-sm" />
                <div className="space-y-2">
                  {filteredInvoices.length === 0 && <div className="text-gray-500 text-sm italic">Aucune facture trouvée.</div>}
          ...
          {previewImage && (
            <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" onClick={()=> setPreviewImage(null)}>
              <div className="bg-white p-4 rounded-lg shadow-xl max-w-3xl w-full relative" onClick={(e)=> e.stopPropagation()}>
                <button onClick={()=> setPreviewImage(null)} className="absolute top-2 right-2 bg-red-500 text-white px-3 py-1 rounded">✕</button>
